---
description: 通过处理和执行tasks.md中定义的所有任务来执行实现计划
---
...
1. 从项目根目录运行 `.specify/scripts/bash/check-prerequisites.sh --json --require-tasks --include-tasks` 并解析FEATURE_DIR和AVAILABLE_DOCS列表。所有路径必须是绝对路径。对于参数中的单引号如"I'm Groot"，使用转义语法：例如 'I'\''m Groot'（或如果可能使用双引号："I'm Groot"）。

2. **检查清单状态**（如果FEATURE_DIR/checklists/存在）：
   - 扫描checklists/目录中的所有清单文件
   - 对于每个清单，计算：
     * 总项目：所有匹配 `- [ ]` 或 `- [X]` 或 `- [x]` 的行
     * 已完成项目：匹配 `- [X]` 或 `- [x]` 的行
     * 未完成项目：匹配 `- [ ]` 的行
   ...
   - **如果任何清单不完整**：
     * 显示带有未完成项目计数的表格
     * **停止**并询问："一些清单不完整。你想继续实现吗？(yes/no)"
     * 在继续之前等待用户响应
     * 如果用户说"no"或"wait"或"stop"，停止执行
     * 如果用户说"yes"或"proceed"或"continue"，继续第3步
...
3. 加载和分析实现上下文：
   - **必需**：读取tasks.md获取完整任务列表和执行计划
   - **必需**：读取plan.md获取技术栈、架构和文件结构
   - **如果存在**：读取data-model.md获取实体和关系
   - **如果存在**：读取contracts/获取API规范和测试要求
   - **如果存在**：读取research.md获取技术决策和约束
   - **如果存在**：读取quickstart.md获取集成场景
...
4. **项目设置验证**：
   ...
   **检测和创建逻辑**：
   - 检查以下命令是否成功以确定仓库是否为git仓库（如果是则创建/验证.gitignore）：
     ...
     ```
   - 检查是否存在Dockerfile*或plan.md中有Docker → 创建/验证.dockerignore
...
6. 按照任务计划执行实现：
   ...
   - **遵循TDD方法**：在相应实现任务之前执行测试任务
   - **基于文件的协调**：影响相同文件的任务必须顺序运行
   - **验证检查点**：在继续之前验证每个阶段的完成

7. 实现执行规则：
   - **设置优先**：初始化项目结构、依赖、配置
   - **测试在代码之前**：如果你需要为合约、实体和集成场景编写测试
...
注意：此命令假设tasks.md中存在完整的任务分解。如果任务不完整或缺失，建议先运行`/tasks`重新生成任务列表。
