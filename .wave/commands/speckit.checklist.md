---
description: 基于用户要求为当前功能生成自定义清单。
---
...
- ✅ "是否为键盘导航定义了无障碍需求？"（覆盖性）
- ✅ "规格是否定义了当徽标图像加载失败时会发生什么？"（边缘案例）

**比喻**：如果您的规格是用英语编写的代码，那么检查清单就是它的单元测试套件。您正在测试需求是否编写良好、完整、明确且准备好实施 - 而不是实施是否有效。
...
2. **澄清意图（动态）**：推导出最多三个初始上下文澄清问题（无预设目录）。它们必须：
   ...
   输出问题（标记 Q1/Q2/Q3）。答案后：如果 ≥2 个场景类（替代/异常/恢复/非功能域）仍不清楚，您可以询问最多两个更有针对性的后续问题（Q4/Q5），每个都有一行理由（例如，"未解决的恢复路径风险"）。不要超过五个总问题。如果用户明确拒绝更多，则跳过升级。

3. **理解用户请求**：结合 `$ARGUMENTS` + 澄清答案：
   - 推导检查清单主题（例如，安全性、审查、部署、用户体验）
   - 整合用户提及的明确必备项目
   - 将焦点选择映射到类别脚手架
...
5. **生成检查清单** - 创建"需求的单元测试"：
   ...
   **如何编写检查清单项目 - "英语的单元测试"**：
   ...
   - "是否用特定尺寸/定位量化了'突出显示'？"[清晰度]
   - "悬停状态需求在所有交互元素中是否一致？"[一致性]
   - "是否为所有交互 UI 定义了键盘导航需求？"[覆盖性]
   - "是否指定了徽标图像加载失败时的后备行为？"[边缘案例]
   ...
   **按质量维度的示例**：
   ...
   覆盖性：
   - "是否为零状态场景（无剧集）定义了需求？[覆盖性, 边缘案例]"
   - "是否处理了并发用户交互场景？[覆盖性, Gap]"
   - "是否为部分数据加载失败指定了需求？[覆盖性, 异常流]"
   
   可衡量性：
   - "视觉层次需求是否可衡量/可测试？[验收标准, Spec §FR-1]"
   - "是否可以客观验证'平衡的视觉重量'？[可衡量性, Spec §FR-2]"

   **场景分类和覆盖**（需求质量焦点）：
   - 检查是否存在以下需求：主要、替代、异常/错误、恢复、非功能场景
   - 对于每个场景类，询问："[场景类型] 需求是否完整、清晰且一致？"
   - 如果缺少场景类："[场景类型] 需求是否有意排除还是缺失？[Gap]"
   ...
   **内容整合**：
   ...
   - 合并检查相同需求方面的近似重复项
   - 如果 > 5 个低影响边缘案例，创建一个项目："需求中是否涵盖了边缘案例 X、Y、Z？[覆盖性]"
...
6. **结构参考**：按照 `.specify/templates/checklist-template.md` 中的规范模板生成检查清单，用于标题、元部分、类别标题和 ID 格式。如果模板不可用，使用：H1 标题、目的/创建元行、包含 `- [ ] CHK### <需求项目>` 行的 `##` 类别部分，从 CHK001 开始全局递增 ID。

7. **报告**：输出创建的检查清单的完整路径、项目数量，并提醒用户每次运行都会创建一个新文件。总结：
   - 选择的焦点区域
   - 深度级别
   - 参与者/时机
   - 合并的任何明确用户指定的必备项目

**重要**：每次 `/speckit.checklist` 命令调用都会使用简短的描述性名称创建检查清单文件，除非文件已经存在。这允许：
...
- "是否为所有交互元素指定了无障碍需求？[覆盖性, Gap]"
- "是否定义了图像加载失败时的后备行为？[边缘案例, Gap]"
...
示例项目：
- "是否为所有受保护资源指定了认证需求？[覆盖性]"
- "是否为敏感信息定义了数据保护需求？[完整性]"
- "是否记录了威胁模型并与需求对齐？[可追溯性]"
- "安全需求是否与合规义务一致？[一致性]"
- "是否定义了安全故障/泄露响应需求？[Gap, 异常流]"
...
- [ ] CHK002 - 测试桌面悬停状态是否正常工作 [Spec §FR-003]
- [ ] CHK003 - 确认徽标点击导航到首页 [Spec §FR-010]
...
- [ ] CHK003 - 是否清晰定义了所有可点击品牌元素的导航需求？[清晰度, Spec §FR-010]
