...
### 4.8 AskUserQuestion — 交互式提问 {#tool-askuser}
...
### 4.9 WebFetch — 网页内容抓取 {#tool-webfetch}
...
### 4.10 ToolSearch — 延迟加载工具发现 {#tool-toolsearch}
...
### 4.11 EnterWorktree / ExitWorktree — Git Worktree 隔离 {#tool-worktree}
...
### 4.12 CronCreate / CronDelete / CronList — 定时任务 {#tool-cron}
...
### 4.13 TaskCreate / TaskGet / TaskUpdate / TaskList — 任务管理 {#tool-task}
...
### 4.14 TaskStop — 中止后台任务 {#tool-taskstop}
...
### 5.1 消息压缩 (Context Management) {#mechanism-context-management}
...
### 5.2 自动记忆系统 (Auto Memory) {#mechanism-auto-memory}
...
### 5.3 记忆规则 (Memory Rules) {#mechanism-memory-rules}
...
### 5.4 后台任务系统 {#mechanism-background-tasks}
...
### 5.5 会话恢复与多会话 {#mechanism-session-restore}
...
### 5.6 文件修改回滚 (Reversion) {#mechanism-reversion}
...
### 7.4 官方插件市场 {#plugin-marketplaces}
...
#### 7.4.6 commit-skills {#plugin-commit-skills}
...
#### 7.4.8 deep-wiki {#plugin-deep-wiki}
...
## 8. 内置 Skills 与 Subagents {#builtin-skills-subagents}

### 8.1 Settings Skill {#settings-skill}
...
### 8.2 其他内置 Skills {#other-builtin-skills}
...
### 8.3 内置 Subagents {#builtin-subagents}
