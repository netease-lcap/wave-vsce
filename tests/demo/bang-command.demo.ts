...
test.describe('Bang Command Demo', () => {
    test('should demonstrate bang command execution and output', async ({ webviewPage }) => {
        ...
        await injector.updateMessages([
            MockDataGenerator.createUserMessage('!sleep 5'),
            MockDataGenerator.createBangMessage('sleep 5', '', true, null)
        ]);
        await webviewPage.screenshot({ path: 'screenshots/bang-command-running.png' });

        // 2. Show a successful command with output
        await injector.updateMessages([
            MockDataGenerator.createUserMessage('!ls -la'),
        ...
        await webviewPage.screenshot({ path: 'screenshots/bang-command-success.png' });

        // 3. Show a long output
        const longOutput = Array.from({ length: 20 }, (_, i) => `line ${i + 1}`).join('\n');
        await injector.updateMessages([
            MockDataGenerator.createUserMessage('!seq 1 20'),
            MockDataGenerator.createBangMessage('seq 1 20', longOutput, false, 0)
        ]);
        await webviewPage.screenshot({ path: 'screenshots/bang-command-long-output.png' });

        // 4. Show a failed command
        await injector.updateMessages([
            MockDataGenerator.createUserMessage('!nonexistent'),
            MockDataGenerator.createBangMessage('nonexistent', 'sh: nonexistent: command not found', false, 127)
        ]);
        await webviewPage.screenshot({ path: 'screenshots/bang-command-failure.png' });
    });
});
